...
class BaseConfig:
    ...
    # `rerank`er specific attributes
    rerank_graph_mode: bool = field(
        default=False,
        metadata={"help": "Whether to use reranker or not."}
    )
    rerank_threshold: float = field(
        default=0.5,
        metadata={"help": "Setting threshold for the reranker."}
    )
    rerank_model_name: str = field(
        default="ms-marco-MiniLM-L6-v2",
        metadata={"help": "Class name indicating which reranker LLM model to use."}
    )
    ...
    ## Dataset running specific attributes -> Corpus
