...
from .reranker import _get_rerank_model_class
...
class HippoRAG:
    ...
    def __init__(self,
        ...
        if rerank_graph_mode is not None:
            self.global_config.rerank_graph_mode = rerank_graph_mode
            self.global_config.rerank_model_name = rerank_model_name
            self.global_config.rerank_threshold = rerank_threshold
        ...
        if self.global_config.rerank_graph_mode is not None:
          self.rerank_model = _get_rerank_model_class(self.global_config)
    ...
    def retrieve(self,
        ...
        for q_idx, query in tqdm(enumerate(queries), desc="Retrieving", total=len(queries)):
            ...
            # 經過ppr後的結果
            # print(">>> [DEBUG] sorted_doc_ids:", sorted_doc_ids)
            ...
            # 若不打算reranking，則直接取前5個
                ...
                retrieval_results_docs = self.rerank_model.rerank(query, sorted_doc_ids, sorted_doc_scores, self.chunk_embedding_store, num_to_retrieve)
            
            # print(">>> [DEBUG] retrieval_results_docs:", retrieval_results_docs)

            retrieval_results.append(QuerySolution(question=query, docs=retrieval_results_docs, doc_scores=sorted_doc_scores[:num_to_retrieve]))
        ...
        # Evaluate retrieval 這邊只是在評分 不用特別改
        if gold_docs is not None:
    ...
    def dense_passage_retrieval(self, query: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # print("query_doc_scores:", query_doc_scores)
        
        # squeeze函數是為了把多餘的維度去掉 在python的世界 即使數學表示為 [1, 3, 1] 但實際上會被視為 [[1, 3, 1]] (2D) 
        # 以squeeze去除多餘維度後 會變成 [1, 3, 1] (1D) => [1, 3, 1] 也就是我們熟悉的陣列，就能夠做熟悉的操作了
        # 但力求嚴謹 盡量不要砍成一維向量來避免 NumPy 的自動推斷可能導致的錯誤。 因為1*3、3*1陣列去squeeze後都會變成3
        query_doc_scores = np.squeeze(query_doc_scores) if query_doc_scores.ndim == 2 else query_doc_scores
        query_doc_scores = min_max_normalize(query_doc_scores)

        # argsort升序(小排到大)並反轉 => 將文件從最相關到最不相關排列。
    ...
    def get_top_k_weights(self,
        ...
        這個函式接受一個包含所有短語權重的 NumPy 陣列和一個包含短語分數的字典，然後根據 link_top_k 這個參數，僅保留得分最高的 $K$ 個實體。
        Args:
        ...
        # 根據分數排序，這邊不會因為不足 K 而報錯，slice的寫法會自動處理
        ...
        # print("top_k_phrases_keys",top_k_phrases_keys)
        # phrases意思是 一個或多個詞語組成的、具有特定意義的片段 = entity  ex: 'george rankin' (人名)、'politician' (職業/類別)、'new york city' (地名)
        
        
        # 這邊是因為整個程式執行是以 self運行 會有保留到上次的結果
        # 所以要把不在 top_k 裡面的 phrase weight 歸零
    ...
    def graph_search_with_fact_entities(self, query: str,
        ...
        for rank, f in enumerate(top_k_facts):
            ...
            # 取得這個triple與我們query的相似度分數 => 這條事實對這個回答有多重要
            fact_score = query_fact_scores[
                top_k_fact_indices[rank]] if query_fact_scores.ndim > 0 else query_fact_scores

            # 找這兩個詞在graph上對應的節點 ID。
        ...
        for phrase, scores in phrase_scores.items():
            linking_score_map[phrase] = float(np.mean(scores)) #(number_of_occurs可能為0 這就導致上面邏輯沒跑所以這邊特地再用np.mean一次)

        # print(">>> [DEBUG] phrase_scores:", phrase_scores)
        # print(">>> [DEBUG] phrase_weights:", phrase_weights)
        # print(">>> [DEBUG] linking_score_map:", linking_score_map)
        # 有了事實權重跟分數後，接下來就可以進行PPR了
        # 這個function算是一個檢查機制而已，確保我們只取 top_k 個實體進行PPR 並把先前self儲存的權重重製掉
        if link_top_k:
        ...
        # 取得 query 轉embedding後對 passage 的 DPR 分數 (後續ppr使用)
        # 這邊回傳回來的已經排序過了 最相關 => 不相關
        dpr_sorted_doc_ids, dpr_sorted_doc_scores = self.dense_passage_retrieval(query)
        normalized_dpr_sorted_scores = min_max_normalize(dpr_sorted_doc_scores)

        # 前面把entity的權重算完了，現在要把 passage 的權重也加進來
        ...
        # print("entity & passage: linking_score_map",linking_score_map)
